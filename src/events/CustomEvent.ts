...
export class CustomEvent<T = void> {
	...
	private allowMultipleListeners: boolean
	private listeners: ((arg?: T) => void)[] = []

	constructor(name: string, multiAllowed?: boolean) {
		this.eventName = name
		this.allowMultipleListeners = multiAllowed
	}

	// Add listener
	add(listener: (arg?: T) => void) {
		this.listeners.push(listener)
		if (this.listeners.length > 1 && !this.allowMultipleListeners) {
			gameConfig.showEventConsole
				? console.warn(`More than one event added to ${this.eventName}`)
				: null
	...
	dispatch(arg?: T) {
		if (gameConfig.showEventConsole) {
			console.log(
				`⚡ DISPATCH : ${this.eventName} (${this.listeners.length} listeners)`
			)
		}
		...
		if (this.listeners.length < 1) {
			if (gameConfig.showEventConsole) {
				...
				console.warn('Event dispatched from:', this._getCallerInfo())
			}
		}
	}
